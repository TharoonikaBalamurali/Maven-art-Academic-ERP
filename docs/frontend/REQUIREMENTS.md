...
| Timetable | `/portal/timetable` | `portal.timetable.view` | **DONE** — the caller's weekly schedule from `/portal/timetable` (§7), laid out as **one table** (Time / Subject / Faculty / Room) with each day as a row group carrying its class count; free days stay listed rather than being omitted. The dashboard's "Today's schedule" rail uses the same tabular format, so the two read alike. Session status (`done` / `now` / `upcoming`) is the backend's — the client never decides which class is in progress from the clock. |
